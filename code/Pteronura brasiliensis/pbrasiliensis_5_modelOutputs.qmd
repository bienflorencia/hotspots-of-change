---
title: "Pteronura brasiliensis Model Outputs"
format: 
  html:
    toc: true
    toc-location: right
...
Model outputs for *Pteronura brasiliensis*.  
...
pbrasiliensis_expert_blob_time1 <- readRDS('data/species_POPA_data/pbrasiliensis_expert_blob_time1.rds')
pbrasiliensis_expert_blob_time2 <- readRDS('data/species_POPA_data/pbrasiliensis_expert_blob_time2.rds')

PA_time1 <- readRDS('data/species_POPA_data/data_pbrasiliensis_PA_time1.rds') %>%
...
PA_time2 <- readRDS('data/species_POPA_data/data_pbrasiliensis_PA_time2.rds') %>%
  cbind(expert=pbrasiliensis_expert_blob_time2) %>% 
  filter(!is.na(env.bio_3) &!is.na(env.bio_5) & !is.na(env.wetland) & !is.na(env.woodysavanna) & !is.na(expert)) # remove NA's

# Presence-only data
pbrasiliensis_expert_gridcell <- readRDS('data/species_POPA_data/pbrasiliensis_expert_gridcell.rds')

PO_time1 <- readRDS('data/species_POPA_data/data_pbrasiliensis_PO_time1.rds') %>%
...
PO_time2  <- readRDS('data/species_POPA_data/data_pbrasiliensis_PO_time2.rds')  %>%
  cbind(expert=pbrasiliensis_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.bio_3) &!is.na(env.bio_5) & !is.na(env.wetland) & !is.na(env.woodysavanna) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
fitted.model <- readRDS('D:/Flo/JAGS_models/pbrasiliensis_model.rds') # deleted after model output
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/pbrasiliensis_model_diagnostics.pdf", param_page=3)
```
...
### Probability of occurrence of *Pteronura brasiliensis*
...
tmap::tmap_save(tm = time1MAP, filename = 'docs/figs/time1MAP_SR_pbrasiliensis.svg', device = svglite::svglite)
tmap::tmap_save(tm = time2MAP, filename = 'docs/figs/time2MAP_SR_pbrasiliensis.svg', device = svglite::svglite)
```

### Standard deviation (SD) of the probability of occurrence of *Pteronura brasiliensis*
...
pbrasiliensis.pal.pu_gn_bivd <- c4a("pu_gn_bivd", n=3, m=5)
pbrasiliensis.pal <- c(t(apply(pbrasiliensis.pal.pu_gn_bivd, 2, rev)))
...
if (length(absent_classes)==0){
  pbrasiliensis.new.pal <- pbrasiliensis.pal
} else pbrasiliensis.new.pal <- pbrasiliensis.pal[-c(absent_classes)]
...
diffMAP.SD <- tm_graticules(alpha = 0.3) + 
  ...
  tm_raster("new_var", style= "cat", palette = pbrasiliensis.new.pal) +
  tm_shape(countries) +
  tm_borders(col='grey60', alpha = 0.4) + 
  tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)

diffMAP.SD
tmap::tmap_save(tm = diffMAP.SD, filename = 'docs/figs/diffMAP_SD_pbrasiliensis.svg', device = svglite::svglite)
```
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_3', 'isothermality',
                                    ifelse(Parameter=='env.bio_5', 'max temperature of warmest month',
                                           ifelse(Parameter=='env.wetland', 'permanent Wetlands',
                                                  ifelse(Parameter=='env.woodysavanna', 'woody savannas', Parameter)))))) %>% 
...
delta.A.plot
ggsave(delta.A.plot, filename='docs/figs/delta_A_pbrasiliensis.svg', device = 'svg', dpi=300)
```
