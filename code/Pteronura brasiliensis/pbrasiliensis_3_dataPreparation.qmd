...
Data preparation for modelling for *speciesLatin*, with the covariates `cov.1`, `cov.2`, `cov.3`, and `cov.4` (selected in the previous step).
...
The following covariates were chosen using the 'variableSelection' script:
 ...
 - `cov.1`: Temperature Annual Range
 - `cov.2`: Mean Temperature of Warmest Quarter
 - `cov.3`: Net Primary Production (cov.3)
 - `cov.4`: Percentage of No Tree Cover
...
cov.3 <- rast(here::here('big_data/cov.3_high.tif'))
bio <- rast(here::here('big_data/bio_high.tif'))
cov.4 <- rast(here::here('big_data/cov.4_high.tif'))
cov.4 <- resample(cov.4, bio, method='bilinear') 
names(cov.4) <- 'cov.4'

# environmental layers for PA
env <- c(bio[[c('cov.1','cov.2')]], cov.4, cov.3) %>% 
...
cov.4.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_unscaled[["cov.4"]]) +
    tm_raster(palette = 'YlGn', midpoint = NA, style= "cont")  + 
    tm_shape(Latam_projected) +
    tm_borders(alpha = 0.3) + 
    tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)

cov.3.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_unscaled[["cov.3"]]) +
    tm_raster(palette = 'BrBG', midpoint = NA, style= "cont")  + 
    tm_shape(Latam_projected) +
    tm_borders(alpha = 0.3) + 
    tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)

cov.1.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_unscaled[["cov.1"]]) +
    tm_raster(palette = 'PuRd', midpoint = NA, style= "cont")  + 
    tm_shape(Latam_projected) +
    tm_borders(alpha = 0.3) + 
    tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)

cov.2.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_unscaled[["cov.2"]]) +
