---
title: "Eira barbara Data Preparation"
format: 
  html:
    toc: true
    toc-location: right
...
PO_ebarbara_time1_raster <- terra::rast('data/PO_ebarbara_time1_raster.tif')
PO_ebarbara_time2_raster <- terra::rast('data/PO_ebarbara_time2_raster.tif')

# blobs
PA_ebarbara_time1_blobs <- readRDS('data/PA_ebarbara_time1_blobs.rds')
PA_ebarbara_time2_blobs <- readRDS('data/PA_ebarbara_time2_blobs.rds')
```
...
PO_ebarbara.countries <- Latam.raster.countries %>% classify(cbind(NaN, NA))
...
PA_ebarbara.env.time1 <- terra::extract(x = env, y = vect(PA_ebarbara_time1_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()

PA_ebarbara.env.time2 <- terra::extract(x = env, y = vect(PA_ebarbara_time2_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()
...
saveRDS(PO_ebarbara_time1.model.data, 'data/data_ebarbara_PO_time1.rds')
saveRDS(PO_ebarbara_time2.model.data, 'data/data_ebarbara_PO_time2.rds')
saveRDS(PA_ebarbara_time1.model.data, 'data/data_ebarbara_PA_time1.rds')
saveRDS(PA_ebarbara_time2.model.data, 'data/data_ebarbara_PA_time2.rds')
```
