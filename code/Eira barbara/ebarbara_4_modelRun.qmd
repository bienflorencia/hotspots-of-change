...
PA_time1 <- readRDS('data/data_ebarbara_PA_time1.rds') %>%
  cbind(expert=ebarbara_expert_blob_time1$presence) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree)) # remove NA's

PA_time2 <- readRDS('data/data_ebarbara_PA_time2.rds') %>%
  cbind(expert=ebarbara_expert_blob_time2$presence) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree)) # remove NA's
...
PO_time1 <- readRDS('data/data_ebarbara_PO_time1.rds') %>%
  cbind(expert=ebarbara_expert_gridcell$expert) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
    ...
    # LIKELIHOOD --------------------------------------------------
      ...
      ## --- Presence-Absence (PA) data ---
       ...
       { 
         ...
         cloglog(psi[i]) <- eta.PA[i] + log(area.PA[i]) + beta*log(effort[i]) + expert.PA[i]
      ...
      for (j in 1:n.PO)
      {
        ...
        nu[j] <- eta.PO[j] + log(area.PO[j]) + expert.PO[j]

        # thinning: the true lambda
        log(lambda[j]) <- ifelse(j > n.PO.half, 
                            nu[j] + P.ret[j], #time1
...
ebarbara_model <- R2jags::jags(data=jags.data,
                                 model.file='models/ebarbara_model.txt',
                                parameters.to.save=c('b', 'P.pred', 
                                            'A.time1', 'A.time2', 'delta.A',
                                            'alpha0', 'alpha1', 'beta',
                                            'lambda', 'P.ret', 'psi',
                                            'y.PO.new', 'r2_tjur', 'auc', 
                                            'delta.Grid'),
                                inits = jags.inits,
                                n.chains=3,
                                n.iter=200000,
...
saveRDS(ebarbara_model, 'big_data/ebarbara_model.rds')
```
