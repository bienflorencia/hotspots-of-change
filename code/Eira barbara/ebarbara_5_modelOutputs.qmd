...
PA_time1 <- readRDS('data/data_ebarbara_PA_time1.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.npp) &!is.na(env.bio_17) & !is.na(env.nontree)) # remove NA's
PA_time2 <- readRDS('data/data_ebarbara_PA_time2.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.npp) &!is.na(env.bio_17) & !is.na(env.nontree)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_ebarbara_PO_time1.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.npp) &!is.na(env.bio_17) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
...
fitted.model <- readRDS('big_data/ebarbara_model.rds') # deleted after model output
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_10', 'covariate 1 name',
                                    ifelse(Parameter=='env.nontree', 'covariate 2 name',
                                           ifelse(Parameter=='env.npp', 'covariate 3 name',
                                                  ifelse(Parameter=='env.bio_17', 'covariate 4 name', Parameter)))))) %>% 
