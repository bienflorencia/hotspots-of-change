---
title: "Eira barbara Model Outputs"
format: 
  html:
    toc: true
    toc-location: right
...
Model outputs for *Eira barbara*.  
...
ebarbara_expert_blob_time1 <- readRDS('data/species_POPA_data/ebarbara_expert_blob_time1.rds')
ebarbara_expert_blob_time2 <- readRDS('data/species_POPA_data/ebarbara_expert_blob_time2.rds')

PA_time1 <- readRDS('data/species_POPA_data/data_ebarbara_PA_time1.rds') %>%
  cbind(expert=ebarbara_expert_blob_time1) %>%
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(expert)) # remove NA's
PA_time2 <- readRDS('data/species_POPA_data/data_ebarbara_PA_time2.rds') %>%
  cbind(expert=ebarbara_expert_blob_time2) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(expert)) # remove NA's

# Presence-only data
ebarbara_expert_gridcell <- readRDS('data/species_POPA_data/ebarbara_expert_gridcell.rds')

PO_time1 <- readRDS('data/species_POPA_data/data_ebarbara_PO_time1.rds') %>%
  cbind(expert=ebarbara_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
PO_time2  <- readRDS('data/species_POPA_data/data_ebarbara_PO_time2.rds')  %>%
  cbind(expert=ebarbara_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
fitted.model <- readRDS('D:/Flo/JAGS_models/ebarbara_model.rds') # deleted after model output
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/ebarbara_model_diagnostics.pdf", param_page=3)
```
...
### Probability of occurrence of *Eira barbara*
...
tmap::tmap_save(tm = time1MAP, filename = 'docs/figs/time1MAP_SR_ebarbara.svg', device = svglite::svglite)
tmap::tmap_save(tm = time2MAP, filename = 'docs/figs/time2MAP_SR_ebarbara.svg', device = svglite::svglite)

```

### Standard deviation (SD) of the probability of occurrence of *Eira barbara*
...
ebarbara.pal.pu_gn_bivd <- c4a("pu_gn_bivd", n=3, m=5)
ebarbara.pal <- c(t(apply(ebarbara.pal.pu_gn_bivd, 2, rev)))
...
if (length(absent_classes)==0){
  ebarbara.new.pal <- ebarbara.pal
} else ebarbara.new.pal <- ebarbara.pal[-c(absent_classes)]
...
diffMAP.SD <- tm_graticules(alpha = 0.3) + 
  ...
  tm_raster("new_var", style= "cat", palette = ebarbara.new.pal) +
  tm_shape(countries) +
  tm_borders(col='grey60', alpha = 0.4) + 
  tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)

diffMAP.SD
tmap::tmap_save(tm = diffMAP.SD, filename = 'docs/figs/diffMAP_SD_ebarbara.svg', device = svglite::svglite)
```
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_10', 'bio_10',
                                    ifelse(Parameter=='env.bio_17', 'bio_17',
                                           ifelse(Parameter=='env.npp', 'npp',
                                                  ifelse(Parameter=='env.nontree', 'nontree', Parameter)))))) %>% 
...
delta.A.plot
ggsave(delta.A.plot, filename='docs/figs/delta_A_ebarbara.svg', device = 'svg', dpi=300)
```
