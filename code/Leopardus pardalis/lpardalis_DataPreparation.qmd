...
Analysis for the *Leopardus pardalis*, with the covariates `bio17`, `bio10`, `npp`, and `tree`.
...
lpardalis_IUCN <- sf::st_read('big_data/ocelot_IUCN.shp', quiet = T) %>% sf::st_transform(crs=equalareaCRS)
...
# Presence-only
data_carnivores_PO %>% filter(species == 'Leopardus pardalis') %>% filter(year>=2000) %>% 
...
# Presence-absence
data_carnivores_PA %>% filter(species == 'Leopardus pardalis' & presence ==1) %>% 
...
# Leopardus pardalis
PA_lpardalis_time1_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time1, 'Leopardus pardalis', 'time1')
PA_lpardalis_time2_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time2, 'Leopardus pardalis', 'time2')
```
...
data_PA_time1_time2 <- rbind(PA_lpardalis_time1_blobs %>% mutate(period='time1'), 
                             PA_lpardalis_time2_blobs %>% mutate(period='time2'))
...
gg_PA_time1 <- ggplot() + 
    ...
    coord_sf(xlim = c(-3500000, 4100000), ylim = c(-4400000, 3000000)) + # st_bbox(lpardalis_IUCN)
...
#        filename='docs/figs/lpardalis_PA_data.svg', device = 'svg', width=10, height=10, dpi=300)
```
...
# Leopardus pardalis
PO_lpardalis_time1_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus pardalis', Latam.raster, 'time1')
PO_lpardalis_time2_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus pardalis', Latam.raster, 'time2')
```
...
#        filename='docs/figs/lpardalis_PO_data.svg', device = 'svg', width=10, height=10, dpi=300)
```
...
tree <- rast('big_data/tree_high.tif')
...
env <- c(bio[[c('bio_17','bio_10')]], tree, npp) %>% 
...
# blobs
saveRDS(PA_lpardalis_time1_blobs, 'data/PA_lpardalis_time1_blobs.rds')
saveRDS(PA_lpardalis_time2_blobs, 'data/PA_lpardalis_time2_blobs.rds')

# presence-only rasters
terra::writeRaster(PO_lpardalis_time1_raster, 'data/PO_lpardalis_time1_raster.tif', overwrite=TRUE)
terra::writeRaster(PO_lpardalis_time2_raster, 'data/PO_lpardalis_time2_raster.tif', overwrite=TRUE)
...
tree.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["tree"]]) +
...
bio_17.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["bio_17"]]) +
...
bio_10.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["bio_10"]]) +
