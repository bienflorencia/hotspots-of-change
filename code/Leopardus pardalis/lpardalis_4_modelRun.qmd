...
PA_time1 <- readRDS('data/data_lpardalis_PA_time1.rds') %>%
  ...
  filter(!is.na(env.bio_12) &!is.na(env.bio_14) & !is.na(env.tree) & !is.na(env.woodysavanna) & !is.na(expert)) # remove NA's
...
PO_time1 <- readRDS('data/data_lpardalis_PO_time1.rds') %>%
  ...
  filter(!is.na(env.bio_12) &!is.na(env.bio_14) & !is.na(env.tree) & !is.na(env.woodysavanna) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
gam.formula <- formula(count ~ env.bio_12 + 
                               env.bio_14 + 
                               env.tree + 
                               env.woodysavanna +
                               expert +
...
PA.X <- cbind('(Intercept)'= 1, 
              'env.bio_12' = PA_time1_time2$env.bio_12,
...
PO.X <- cbind('(Intercept)'= 1, 
              'env.bio_12' = PO_time1_time2$env.bio_12,
...
saveRDS(lpardalis_model, 'D:/Flo/JAGS_models/lpardalis_model.rds')
```
