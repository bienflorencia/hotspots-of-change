---
title: "Leopardus pardalis Model Run"
format: 
  html:
    toc: true
    toc-location: right
...
Model run for *Leopardus pardalis* with the data prepared in the previous step (including species and covariates data).  
...
lpardalis_expert_blob_time1 <- readRDS('data/lpardalis_expert_blob_time1.rds')
lpardalis_expert_blob_time2 <- readRDS('data/lpardalis_expert_blob_time2.rds')

PA_time1 <- readRDS('data/data_lpardalis_PA_time1.rds') %>%
  cbind(expert=lpardalis_expert_blob_time1$presence) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.tree) & !is.na(env.npp)) # remove NA's

PA_time2 <- readRDS('data/data_lpardalis_PA_time2.rds') %>%
  cbind(expert=lpardalis_expert_blob_time2$presence) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.tree) & !is.na(env.npp)) # remove NA's

# Presence-only data
lpardalis_expert_gridcell <- readRDS('data/lpardalis_expert_gridcell.rds')

PO_time1 <- readRDS('data/data_lpardalis_PO_time1.rds') %>%
  cbind(expert=lpardalis_expert_gridcell$expert) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.tree) & !is.na(env.npp) & !is.na(acce) & !is.na(count)) # remove NA's
PO_time2  <- readRDS('data/data_lpardalis_PO_time2.rds') %>%
  cbind(expert=lpardalis_expert_gridcell$expert) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_17) & !is.na(env.tree) & !is.na(env.npp) & !is.na(acce) & !is.na(count)) # remove NA's
...
PA.X <- cbind('(Intercept)'= 1, 
              ...
              'env.tree' = PA_time1_time2$env.tree,
...
PO.X <- cbind('(Intercept)'= 1, 
              ...
              'env.tree' = PO_time1_time2$env.tree,
...
', file = 'models/lpardalis_model.txt')
```
...
lpardalis_model <- R2jags::jags(data=jags.data,
                                 model.file='models/lpardalis_model.txt',
...
saveRDS(lpardalis_model, 'D:/Flo/HNC_models/lpardalis_model.rds')
```
