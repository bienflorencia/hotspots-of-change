...
PA_time1 <- readRDS('data/data_lpardalis_PA_time1.rds') %>%
  ...
  filter(!is.na(env.bio_12) &!is.na(env.bio_14) & !is.na(env.woodysavanna) & !is.na(env.tree) & !is.na(expert)) # remove NA's
...
PO_time1 <- readRDS('data/data_lpardalis_PO_time1.rds') %>%
  ...
  filter(!is.na(env.bio_12) &!is.na(env.bio_14) & !is.na(env.woodysavanna) & !is.na(env.tree) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_12', 'annual precipitation',
                                    ifelse(Parameter=='env.tree', 'percentage of tree cover',
                                           ifelse(Parameter=='env.woodysavanna', 'percentage of woody savanna',
                                                  ifelse(Parameter=='env.bio_14', 'precipitation of driest month', Parameter)))))) %>% 
