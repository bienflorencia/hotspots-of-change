...
PA_time1 <- readRDS('data/data_lpardalis_PA_time1.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.tree) &!is.na(env.bio_17) & !is.na(env.npp)) # remove NA's
PA_time2 <- readRDS('data/data_lpardalis_PA_time2.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.tree) &!is.na(env.bio_17) & !is.na(env.npp)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lpardalis_PO_time1.rds') %>% 
  filter(!is.na(env.bio_10) &!is.na(env.tree) &!is.na(env.bio_17) & !is.na(env.npp) & !is.na(acce) & !is.na(count)) # remove NA's
...
L.fitted.model.b <- plab("b", 
                         list(Covariate = c('Intercept', 
                                            ...
                                            'env.bio_17',
                                            'env.tree', 
                                            'env.npp', sprintf('spline%i', 1:18)))) # changes with n.spl
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_10', 'mean temperature of the warmest quarter',
                                    ifelse(Parameter=='env.npp', 'npp',
                                           ifelse(Parameter=='env.tree', 'percentage of tree cover',
                                                  ifelse(Parameter=='env.bio_17', 'precipitation of the driest quarter', Parameter)))))) %>% 
