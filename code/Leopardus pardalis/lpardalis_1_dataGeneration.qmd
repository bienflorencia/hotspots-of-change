---
title: "Leopardus pardalis Data Generation"
format: 
  html:
    toc: true
    toc-location: right
...
Data generation for *Leopardus pardalis* (blobs and rasters).
...
data_carnivores_PO %>% filter(species == 'Leopardus pardalis') %>% filter(year>=2000) %>% 
  mutate(period=ifelse(year<2014, 'first period: 2000-2013', 'second period: 2014-2021')) %>%
...
data_carnivores_PA %>% filter(species == 'Leopardus pardalis' & presence ==1) %>% 
  mutate(period=ifelse(year(date_start)<2014, 'first period: 2000-2013', 'second period: 2014-2021')) %>%
  group_by(species, period) %>% count() %>% pivot_wider(names_from = 'period', values_from = n) %>% kable()
```
...
# Leopardus pardalis
PO_lpardalis_time1_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus pardalis', latam_raster, 'time1')
PO_lpardalis_time2_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus pardalis', latam_raster, 'time2')
```
...
# blobs
saveRDS(PA_lpardalis_time1_blobs, 'data/species_POPA_data/PA_lpardalis_time1_blobs.rds')
saveRDS(PA_lpardalis_time2_blobs, 'data/species_POPA_data/PA_lpardalis_time2_blobs.rds')

# presence-only rasters
terra::writeRaster(PO_lpardalis_time1_raster, 'data/species_POPA_data/PO_lpardalis_time1_raster.tif', overwrite=TRUE)
terra::writeRaster(PO_lpardalis_time2_raster, 'data/species_POPA_data/PO_lpardalis_time2_raster.tif', overwrite=TRUE)

saveRDS(data_PA_time1_time2, 'data/species_POPA_data/data_lpardalis_PA.rds')
saveRDS(data_PO_time1_time2, 'data/species_POPA_data/data_lpardalis_PO.rds')
```
