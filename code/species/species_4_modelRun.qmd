...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>%
  filter(!is.na(env.npp) &!is.na(env.bio_4) & !is.na(env.bio_7) & !is.na(env.nontree)) # remove NA's
PA_time2 <- readRDS('data/data_lwiedii_PA_time2.rds') %>%
  filter(!is.na(env.npp) &!is.na(env.bio_4) & !is.na(env.bio_7) & !is.na(env.nontree)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>%
  filter(!is.na(env.npp) &!is.na(env.bio_4) & !is.na(env.bio_7) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
