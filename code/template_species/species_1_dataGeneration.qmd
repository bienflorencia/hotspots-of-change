...
Data generation for *SPECIES* (blobs and rasters).
...
# Presence-only
data_carnivores_PO %>% filter(species == 'SPECIES') %>% filter(year>=2000) %>% 
...
# Presence-absence
data_carnivores_PA %>% filter(species == 'SPECIES' & presence ==1) %>% 
...
# SPECIES
PA_SPECIES_time1_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time1, 'SPECIES', 'time1')
PA_SPECIES_time2_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time2, 'SPECIES', 'time2')
```
...
data_PA_time1_time2 <- rbind(PA_SPECIES_time1_blobs %>% mutate(period='time1'), 
                             PA_SPECIES_time2_blobs %>% mutate(period='time2'))

gg_PA_time1 <- ggplot() + 
    geom_sf(data=Latam_countries, fill='#fafaf8', col='#bfc5c7', size=0.2) +
    geom_sf(data=SPECIES_IUCN, fill='grey60', alpha=0.5, col='grey90', size=0.1) +
    geom_sf(data=data_PA_time1_time2 %>% filter(period=='time1') %>% st_buffer(20000), 
            aes(fill=factor(presence)), col=NA, show.legend = F) +
    coord_sf(xlim = c(-3500000, 4100000), ylim = c(-4400000, 3000000)) + # st_bbox(SPECIES_IUCN)
...
# SPECIES
PO_SPECIES_time1_raster <- calculate_PO_sp_period_raster(PO, 'SPECIES', Latam.raster, 'time1')
PO_SPECIES_time2_raster <- calculate_PO_sp_period_raster(PO, 'SPECIES', Latam.raster, 'time2')
```
...
# blobs
saveRDS(PA_SPECIES_time1_blobs, 'data/PA_SPECIES_time1_blobs.rds')
saveRDS(PA_SPECIES_time2_blobs, 'data/PA_SPECIES_time2_blobs.rds')

# presence-only rasters
terra::writeRaster(PO_SPECIES_time1_raster, 'data/PO_SPECIES_time1_raster.tif', overwrite=TRUE)
terra::writeRaster(PO_SPECIES_time2_raster, 'data/PO_SPECIES_time2_raster.tif', overwrite=TRUE)
```
