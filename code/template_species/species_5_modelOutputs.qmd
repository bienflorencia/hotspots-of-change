...
PA_time1 <- readRDS('data/data_SPECIES_PA_time1.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4)) # remove NA's
PA_time2 <- readRDS('data/data_SPECIES_PA_time2.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_SPECIES_PO_time1.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4) & !is.na(acce) & !is.na(count)) # remove NA's
PO_time2  <- readRDS('data/data_SPECIES_PO_time2.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4) & !is.na(acce) & !is.na(count)) # remove NA's
...
fitted.model <- readRDS('big_data/SPECIES_model.rds') # deleted after model output
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/SPECIES_model_diagnostics.pdf", param_page=3)
```
...
### Standard deviation (SD) of the probability of occurrence of *SPECIES*
...
SPECIES.pal.pu_gn_bivd <- c4a("pu_gn_bivd", n=3, m=5)
SPECIES.pal <- c(t(apply(SPECIES.pal.pu_gn_bivd, 2, rev)))
...
if (length(absent_classes)==0){
  SPECIES.new.pal <- SPECIES.pal
} else SPECIES.new.pal <- SPECIES.pal[-c(absent_classes[i])]
...
diffMAP.SD <- tm_graticules(alpha = 0.3) + 
  ...
  tm_raster("new_var", style= "cat", palette = SPECIES.new.pal) +
  tm_shape(Latam_countries) +
  tm_borders(col='grey60', alpha = 0.4) + 
  tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)
