---
title: "speciesLatin Model Outputs"
format: 
  html:
    toc: true
    toc-location: right
...
Model outputs for *speciesLatin*.  
...
speciesAbrev_expert_blob_time1 <- readRDS('data/speciesAbrev_expert_blob_time1.rds')
speciesAbrev_expert_blob_time2 <- readRDS('data/speciesAbrev_expert_blob_time2.rds')

PA_time1 <- readRDS('data/data_speciesAbrev_PA_time1.rds') %>%
  cbind(expert=speciesAbrev_expert_blob_time1) %>%
  filter(!is.na(env.cov.1) &!is.na(env.cov.2) & !is.na(env.cov.3) & !is.na(env.cov.4) & !is.na(expert)) # remove NA's
PA_time2 <- readRDS('data/data_speciesAbrev_PA_time2.rds') %>%
  cbind(expert=speciesAbrev_expert_blob_time2) %>% 
  filter(!is.na(env.cov.1) &!is.na(env.cov.2) & !is.na(env.cov.3) & !is.na(env.cov.4) & !is.na(expert)) # remove NA's

# Presence-only data
speciesAbrev_expert_gridcell <- readRDS('data/speciesAbrev_expert_gridcell.rds')

PO_time1 <- readRDS('data/data_speciesAbrev_PO_time1.rds') %>%
  cbind(expert=speciesAbrev_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.cov.1) &!is.na(env.cov.2) & !is.na(env.cov.3) & !is.na(env.cov.4) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
PO_time2  <- readRDS('data/data_speciesAbrev_PO_time2.rds')  %>%
  cbind(expert=speciesAbrev_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.cov.1) &!is.na(env.cov.2) & !is.na(env.cov.3) & !is.na(env.cov.4) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
fitted.model <- readRDS('big_data/speciesAbrev_model.rds') # deleted after model output
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/speciesAbrev_model_diagnostics.pdf", param_page=3)
```
...
### Probability of occurrence of *speciesLatin*
...
### Standard deviation (SD) of the probability of occurrence of *speciesLatin*
...
speciesAbrev.pal.pu_gn_bivd <- c4a("pu_gn_bivd", n=3, m=5)
speciesAbrev.pal <- c(t(apply(speciesAbrev.pal.pu_gn_bivd, 2, rev)))
...
if (length(absent_classes)==0){
  speciesAbrev.new.pal <- speciesAbrev.pal
} else speciesAbrev.new.pal <- speciesAbrev.pal[-c(absent_classes)]
...
diffMAP.SD <- tm_graticules(alpha = 0.3) + 
  ...
  tm_raster("new_var", style= "cat", palette = speciesAbrev.new.pal) +
  tm_shape(Latam_countries) +
  tm_borders(col='grey60', alpha = 0.4) + 
  tm_layout(legend.outside = T, frame.lwd = 0.3, scale=1.2, legend.outside.size = 0.1)
...
countries <- countryLevels %>% filter(value %in% rasterLevels) %>% mutate(numLevel=1:length(rasterLevels))
...
country_acce <- bind_rows(ci.alpha[length(rasterLevels)+1,], 
          tibble(countries, ci.alpha[1:length(rasterLevels),])) %>% 
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.cov.1', 'name cov.1',
                                    ifelse(Parameter=='env.cov.4', 'name cov.2',
                                           ifelse(Parameter=='env.cov.3', 'name cov.3',
                                                  ifelse(Parameter=='env.cov.2', 'name cov.4', Parameter)))))) %>% 
