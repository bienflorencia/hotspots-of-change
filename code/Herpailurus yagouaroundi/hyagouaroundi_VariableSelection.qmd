---
title: "Herpailurus yagouaroundi Variable Selection"
format: 
  html:
    toc: true
    toc-location: right
...
### Yaguarundí
  ...
  -   It occurs in a variety of habitats, from closed primary rainforest to open desert, scrub, and grassland, although in open areas it sticks to patches of dense cover. 
  -   Caso and Tewes (2007) found that radio-collared jaguarundis used mature forest 53% of the time and pasture-grassland 47% of the time. It is predominantly a lowland species ranging up to 2000 m, although it was reported up to 3200 m in Colombia (Cuervo et al. 1986). 
  -   Yaguarundís occur at relatively low densities of 1–5/100 km2 in Brazil (Oliveira et al. 2008), but reach densities up to 20/100 km2 in Tamaulipas, Mexico. Although with its wide range the species is classified as Least Concern, habitat loss is a threat, especially for the more open types, which are being converted to large-scale agriculture (IUCN 2008).


## Species data

We will use the data for the second period (2014-2020)
...
PA_time1 <- readRDS('data/PA_hyagouaroundi_time1_blobs.rds')%>% ungroup()
PA_time2 <- readRDS('data/PA_hyagouaroundi_time2_blobs.rds')%>% ungroup()
...
Boosted regression trees were fitted using packages: gbm and dismo—specifically the gbm.step function in dismo (Hijmans et al., 2017)
