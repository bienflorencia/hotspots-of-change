...
PA_time1 <- readRDS('data/data_hyagouaroundi_PA_time1.rds') %>%
  cbind(expert=hyagouaroundi_expert_blob_time1$presence) %>%
  filter(!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.npp) & !is.na(env.elev)) # remove NA's
PA_time2 <- readRDS('data/data_hyagouaroundi_PA_time2.rds') %>%
  cbind(expert=hyagouaroundi_expert_blob_time2$presence) %>% 
  filter(!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.npp) & !is.na(env.elev)) # remove NA's
...
PO_time1 <- readRDS('data/data_hyagouaroundi_PO_time1.rds') %>%
  cbind(expert=hyagouaroundi_expert_gridcell$expert) %>% 
  filter(!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.npp) & !is.na(env.elev) & !is.na(acce) & !is.na(count)) # remove NA's
  ...
  { 
    ...
    # LIKELIHOOD --------------------------------------------------
      ...
      for (j in 1:n.PO)
      {
        ...
        nu[j] <- exp(eta.PO[j]) * area.PO[j] 

        # thinning: the true lambda
        lambda[j] <- ifelse(j > n.PO.half,
                            nu[j] * P.ret[j] * expert.PO[j], #time1
                            nu[j] * P.ret[j] * expert.PO[j] * global.effort[j]) #time2
    ...
    for (j in 1:n.PO)
    {
      ...
      cloglog(P.pred[j]) <- eta.pred[j] + log(area.PO[j]) + expert.PO[j]
    }
