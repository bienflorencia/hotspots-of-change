---
title: "Herpailurus yagouaroundi Model Outputs"
format: 
  html:
    toc: true
    toc-location: right
...
PA_time1 <- readRDS('data/data_yaguarundi_PA_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.elev)) # remove NA's
PA_time2 <- readRDS('data/data_yaguarundi_PA_time2.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.elev)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_yaguarundi_PO_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.elev) & !is.na(acce) & !is.na(count)) # remove NA's
PO_time2  <- readRDS('data/data_yaguarundi_PO_time2.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_15) & !is.na(env.elev) & !is.na(acce) & !is.na(count)) # remove NA's
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/hyagouaroundi_model_diagnostics.pdf", param_page=3)
```
...
# tmap_save(time1MAP, filename='docs/figs/hyagouaroundi_time1MAP.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
# tmap_save(time2MAP, filename='docs/figs/hyagouaroundi_time2MAP.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
# tmap_save(diffMAP, filename='docs/figs/hyagouaroundi_diffMAP.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
```
...
# tmap_save(time1MAP.sd, filename='docs/figs/hyagouaroundi_time1MAPsd.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
# tmap_save(time2MAP.sd, filename='docs/figs/hyagouaroundi_time2MAPsd.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
```
...
# tmap_save(delta.GridMAP.sd, filename='docs/figs/hyagouaroundi_deltaGridMAPsd.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
...
# tmap_save(diffMAP.SD, filename='docs/figs/hyagouaroundi_diffMAPSD.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
```
...
# ggsave(plot = caterpiller.params, filename='docs/figs/hyagouaroundi_caterpiller.params.svg', device = 'svg', width=6, height=5, dpi=300)
```

### Boxplot of time2erior densities of the predicted area in both time periods
...
# ggsave(plot = range.boxplot, filename='docs/figs/hyagouaroundi_range.boxplot.svg', device = 'svg', width=5, height=5, dpi=300)
...
range.ci
# ggsave(plot = range.ci, filename='docs/figs/hyagouaroundi_range.ci.svg', device = 'svg', width=5, height=5, dpi=300)
```


### time2erior distribution of range change (Area).
...
# ggsave(plot = delta.A.plot, filename='docs/figs/hyagouaroundi_delta.A.plot.svg', device = 'svg', width=5, height=5, dpi=300)
```


## time2erior predictive checks
...
```{r}
#| label: time2erior-predictive-PO
#| echo: true
#| eval: true
#| message: false
#| warning: false
...
# ggsave(plot = pp.PO | pp.PO.log10, filename='docs/figs/hyagouaroundi_pp.PO.svg', device = 'svg', width=7, height=5, dpi=300)
```
...
- Calculate AUC from time2erior medians of sensitivity and 1-specificity
- Compare to time2erior median of AUC
