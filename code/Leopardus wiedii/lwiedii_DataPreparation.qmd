...
Analysis for the *Leopardus wiedii*, with the covariates `bio7`, `bio12`, `npp`, and `nonveg`.
...
# Presence-only
data_carnivores_PO %>% filter(species == 'Leopardus wiedii') %>% filter(year>=2000) %>% 
...
# Presence-absence
data_carnivores_PA %>% filter(species == 'Leopardus wiedii' & presence ==1) %>% 
...
# Leopardus wiedii
PA_lwiedii_time1_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time1, 'Leopardus wiedii', 'time1')
PA_lwiedii_time2_blobs <- calculate_PA_sp_blobs(PA, blobs_efforttime_time2, 'Leopardus wiedii', 'time2')
```
...
data_PA_time1_time2 <- rbind(PA_lwiedii_time1_blobs %>% mutate(period='time1'), 
                             PA_lwiedii_time2_blobs %>% mutate(period='time2'))
...
gg_PA_time1 <- ggplot() + 
    ...
    coord_sf(xlim = c(-3500000, 4100000), ylim = c(-4400000, 3000000)) + # st_bbox(lwiedii_IUCN)
...
#        filename='docs/figs/lwiedii_PA_data.svg', device = 'svg', width=10, height=10, dpi=300)
```
...
# Leopardus wiedii
PO_lwiedii_time1_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus wiedii', Latam.raster, 'time1')
PO_lwiedii_time2_raster <- calculate_PO_sp_period_raster(PO, 'Leopardus wiedii', Latam.raster, 'time2')
```
...
#        filename='docs/figs/lwiedii_PO_data.svg', device = 'svg', width=10, height=10, dpi=300)
```
...
## Covariates data
 ...
 - `bio12`: Precipitation Seasonality
...
nonveg <- rast(here::here('big_data/nonveg_high.tif'))
nonveg <- resample(nonveg, bio, method='bilinear') 
names(nonveg) <- 'nonveg'
...
env <- c(bio[[c('bio_7','bio_12')]], nonveg, npp) %>% 
...
PO_lwiedii.countries <- Latam.raster.countries %>% classify(cbind(NaN, NA))
...
saveRDS(PO_lwiedii_time1.model.data, 'data/data_lwiedii_PO_time1.rds')
saveRDS(PO_lwiedii_time2.model.data, 'data/data_lwiedii_PO_time2.rds')
saveRDS(PA_lwiedii_time1.model.data, 'data/data_lwiedii_PA_time1.rds')
saveRDS(PA_lwiedii_time2.model.data, 'data/data_lwiedii_PA_time2.rds')

# blobs
saveRDS(PA_lwiedii_time1_blobs, 'data/PA_lwiedii_time1_blobs.rds')
saveRDS(PA_lwiedii_time2_blobs, 'data/PA_lwiedii_time2_blobs.rds')

# presence-only rasters
terra::writeRaster(PO_lwiedii_time1_raster, 'data/PO_lwiedii_time1_raster.tif', overwrite=TRUE)
terra::writeRaster(PO_lwiedii_time2_raster, 'data/PO_lwiedii_time2_raster.tif', overwrite=TRUE)
...
nonveg.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["nonveg"]]) +
...
bio_12.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["bio_12"]]) +
