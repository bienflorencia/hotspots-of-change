...
Model run for *Leopardus wiedii* with the data prepared in the previous step (including species and covariates data).  
...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>%
  filter(!is.na(env.bio_7) &!is.na(env.bio_9) & !is.na(env.npp) & !is.na(env.nontree)) # remove NA's
PA_time2 <- readRDS('data/data_lwiedii_PA_time2.rds') %>%
  filter(!is.na(env.bio_7) &!is.na(env.bio_9) & !is.na(env.npp) & !is.na(env.nontree)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>%
  filter(!is.na(env.bio_7) &!is.na(env.bio_9) & !is.na(env.npp) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
...
PA.X <- cbind('(Intercept)'= 1, 
              ...
              'env.bio_9' = PA_time1_time2$env.bio_9,
...
PO.X <- cbind('(Intercept)'= 1, 
              ...
              'env.bio_9' = PO_time1_time2$env.bio_9,
...
saveRDS(lwiedii_model, 'big_data/lwiedii_model.rds')
```
