---
title: "Leopardus wiedii Model Run"
format: 
  html:
    toc: true
    toc-location: right
...
Analysis for the *Leopardus wiedii*, with new covariates: `npp`, `nonveg`, `bio7`, and `bio12`.
...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>%
  filter(!is.na(env.npp) &!is.na(env.bio_12) & !is.na(env.bio_7) & !is.na(env.nonveg)) # remove NA's
...
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>%
  filter(!is.na(env.npp) &!is.na(env.bio_12) & !is.na(env.bio_7) & !is.na(env.nonveg) & !is.na(acce) & !is.na(count)) # remove NA's
...
PA.X <- cbind('(Intercept)'= 1, 
              ...
              'env.nonveg' = PA_time1_time2$env.nonveg,
              jagam.PA.time1, 
              jagam.PA.time2)
...
PO.X <- cbind('(Intercept)'= 1, 
              ...
              'env.nonveg' = PO_time1_time2$env.nonveg,
              jagam.PO.time1, 
              jagam.PO.time2)
...
# jags.data <- readRDS('data/tmp_jags_data.rds')

lwiedii_model <- R2jags::jags(data=jags.data,
                                 model.file='models/lwiedii_model.txt',
...
saveRDS(lwiedii_model, 'D:/Flo/HNC_models/lwiedii_model.rds')
