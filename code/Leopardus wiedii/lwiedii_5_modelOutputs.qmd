...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>% 
  filter(!is.na(env.bio_7) &!is.na(env.npp) &!is.na(env.bio_10) & !is.na(env.nontree)) # remove NA's
PA_time2 <- readRDS('data/data_lwiedii_PA_time2.rds') %>% 
  filter(!is.na(env.bio_7) &!is.na(env.npp) &!is.na(env.bio_10) & !is.na(env.nontree)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>% 
  filter(!is.na(env.bio_7) &!is.na(env.npp) &!is.na(env.bio_10) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
...
fitted.model <- readRDS('big_data/lwiedii_model.rds') # deleted after model output
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_7', 'temperature annual range',
                                    ifelse(Parameter=='env.nontree', 'percentage of no tree cover',
                                           ifelse(Parameter=='env.npp', 'net primary production',
                                                  ifelse(Parameter=='env.bio_10', 'mean temperature of warmest quarter', Parameter)))))) %>% 
