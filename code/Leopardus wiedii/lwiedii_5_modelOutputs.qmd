...
PA_time1 <- readRDS('data/data_speciesAbrev_PA_time1.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4)) # remove NA's
PA_time2 <- readRDS('data/data_speciesAbrev_PA_time2.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_speciesAbrev_PO_time1.rds') %>% 
  filter(!is.na(cov.1) &!is.na(cov.3) &!is.na(cov.2) & !is.na(cov.4) & !is.na(acce) & !is.na(count)) # remove NA's
...
L.fitted.model.b <- plab("b", 
                         list(Covariate = c('Intercept', 
                                            ...
                                            'cov.2',
                                            'cov.3', 
                                            'cov.4', sprintf('spline%i', 1:18)))) # changes with n.spl
...
} else speciesAbrev.new.pal <- speciesAbrev.pal[-c(absent_classes[i])]
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='cov.1', 'covariate 1 name',
                                    ifelse(Parameter=='cov.4', 'covariate 2 name',
                                           ifelse(Parameter=='cov.3', 'covariate 3 name',
                                                  ifelse(Parameter=='cov.2', 'covariate 4 name', Parameter)))))) %>% 
