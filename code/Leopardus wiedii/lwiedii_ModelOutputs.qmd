...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_12) & !is.na(env.nonveg)) # remove NA's
PA_time2 <- readRDS('data/data_lwiedii_PA_time2.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_12) & !is.na(env.nonveg)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_12) & !is.na(env.nonveg) & !is.na(acce) & !is.na(count)) # remove NA's
...
L.fitted.model.b <- plab("b", 
                         list(Covariate = c('Intercept', 
                                            ...
                                            'env.bio_12',
                                            'env.bio_7', 
                                            'env.nonveg', sprintf('spline%i', 1:18)))) # changes with n.spl
...
lwiedii.new.pal <- lwiedii.pal[-c(absent_classes)]
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.npp', 'net primary production',
                                    ifelse(Parameter=='env.nonveg', 'percentage of non tree vegetation cover',
                                           ifelse(Parameter=='env.bio_7', 'temperature annual range',
                                                  ifelse(Parameter=='env.bio_12', 'annual precipitation', Parameter)))))) %>% 
