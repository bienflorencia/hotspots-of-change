...
lwiedii_IUCN <- sf::st_read('big_data/margay_IUCN.shp', quiet = T) %>% sf::st_transform(crs=equalareaCRS)
...
PA_time1 <- readRDS('data/data_lwiedii_PA_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_4) & !is.na(env.nontree)) # remove NA's
PA_time2 <- readRDS('data/data_lwiedii_PA_time2.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_4) & !is.na(env.nontree)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_lwiedii_PO_time1.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_4) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
PO_time2  <- readRDS('data/data_lwiedii_PO_time2.rds') %>% 
  filter(!is.na(env.npp) &!is.na(env.bio_7) &!is.na(env.bio_4) & !is.na(env.nontree) & !is.na(acce) & !is.na(count)) # remove NA's
...
fitted.model <- readRDS('D:/Flo/HNC_models/lwiedii_model.rds') # deleted after model output
...
L.fitted.model.b <- plab("b", 
                         list(Covariate = c('Intercept', 
                                            ...
                                            'env.bio_4',
                                            'env.bio_7', 
                                            'env.nontree', sprintf('spline%i', 1:18)))) # changes with n.spl
...
### Probability of occurrence of the Leopardus wiedii
...
# tmap_save(time1MAP, filename='docs/figs/lwiedii_time1MAP.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
# tmap_save(time2MAP, filename='docs/figs/lwiedii_time2MAP.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
# tmap_save(diffMAP, filename='docs/figs/lwiedii_diffMAP.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
```
...
# tmap_save(time1MAP.sd, filename='docs/figs/lwiedii_time1MAPsd.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
# tmap_save(time2MAP.sd, filename='docs/figs/lwiedii_time2MAPsd.svg', dpi=300,  height = 7, width = 6, device = svglite::svglite)
```
...
# tmap_save(delta.GridMAP.sd, filename='docs/figs/lwiedii_deltaGridMAPsd.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
...
} else lwiedii.new.pal <- lwiedii.pal[-c(absent_classes[i])]
...
# tmap_save(diffMAP.SD, filename='docs/figs/lwiedii_diffMAPSD.svg', dpi=300, height = 7, width = 6, device = svglite::svglite)
```
...
# ggsave(plot = acce_country, filename='../docs/figs/acce_country.svg', 
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.npp', 'net primary production',
                                    ifelse(Parameter=='env.nontree', 'percentage of no tree cover',
                                           ifelse(Parameter=='env.bio_7', 'temperature annual range',
                                                  ifelse(Parameter=='env.bio_4', 'mean temperature of warmest quarter', Parameter)))))) %>% 
...
# ggsave(plot = caterpiller.params, filename='docs/figs/lwiedii_caterpiller.params.svg', device = 'svg', width=6, height=5, dpi=300)
```
...
# ggsave(plot = range.boxplot, filename='docs/figs/lwiedii_range.boxplot.svg', device = 'svg', width=5, height=5, dpi=300)
...
range.ci
# ggsave(plot = range.ci, filename='docs/figs/lwiedii_range.ci.svg', device = 'svg', width=5, height=5, dpi=300)
```
...
# ggsave(plot = delta.A.plot, filename='docs/figs/lwiedii_delta.A.plot.svg', device = 'svg', width=5, height=5, dpi=300)
```
...
# ggsave(plot = pp.PO | pp.PO.log10, filename='docs/figs/lwiedii_pp.PO.svg', device = 'svg', width=7, height=5, dpi=300)
```
...
# ggsave(plot = pp.PA, filename='../docs/figs/pp.PA.svg', device = 'svg', width=5, height=5, dpi=300)
```

#### AUC

Source: [Calculate AUC in a JAGS model](https://gist.github.com/johnbaums/098bb760422801d10aed)
...
sens <- jagsresults(fitted.model, 'sens')[, '50%']
fpr <- jagsresults(fitted.model, 'fpr')[, '50%']

plot(fpr, sens, type='o', pch=20, xlab='1 - Specificity', ylab='Sensitivity')
sum((sens[-1]+sens[-length(sens)])/2 * -diff(fpr)) # auc

# 
jagsresults(fitted.model, 'auc')[, '50%']
...
# ggsave(plot = grid.level.change, filename='../docs/figs/grid.level.change.svg', device = 'svg', width=8, height=8, dpi=300)
