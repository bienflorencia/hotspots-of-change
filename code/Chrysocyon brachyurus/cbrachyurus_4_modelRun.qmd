---
title: "Chrysocyon brachyurus Model Run"
format: 
  html:
    toc: true
    toc-location: right
...
PA_time1 <- readRDS('data/data_cbrachyurus_PA_time1.rds') %>%
  ...
  filter(!is.na(env.bio_5) &!is.na(env.bio_10) & !is.na(env.elev) & !is.na(env.grass) & !is.na(expert)) # remove NA's
...
PO_time1 <- readRDS('data/data_cbrachyurus_PO_time1.rds') %>%
  ...
  filter(!is.na(env.bio_5) &!is.na(env.bio_10) & !is.na(env.elev) & !is.na(env.grass) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
gam.formula <- formula(count ~ env.bio_5 + 
                               env.bio_10 + 
                               env.elev + 
                               env.grass +
                               expert +
...
cbrachyurus_model <- R2jags::jags(data=jags.data,
                                model.file='models/cbrachyurus_model.txt',
...
saveRDS(cbrachyurus_model, 'big_data/cbrachyurus_model.rds')
```
