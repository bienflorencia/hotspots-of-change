---
title: "Chrysocyon brachyurus Model Outputs"
format: 
  html:
    toc: true
    toc-location: right
...
cbrachyurus_expert_blob_time1 <- readRDS('data/species_POPA_data/cbrachyurus_expert_blob_time1.rds')
cbrachyurus_expert_blob_time2 <- readRDS('data/species_POPA_data/cbrachyurus_expert_blob_time2.rds')

PA_time1 <- readRDS('data/species_POPA_data/data_cbrachyurus_PA_time1.rds') %>%
  cbind(expert=cbrachyurus_expert_blob_time1) %>%
  filter(!is.na(env.bio_10) &!is.na(env.bio_14) & !is.na(env.elev) & !is.na(env.grass) & !is.na(expert)) # remove NA's
PA_time2 <- readRDS('data/species_POPA_data/data_cbrachyurus_PA_time2.rds') %>%
  cbind(expert=cbrachyurus_expert_blob_time2) %>% 
  filter(!is.na(env.bio_10) &!is.na(env.bio_14) & !is.na(env.elev) & !is.na(env.grass) & !is.na(expert)) # remove NA's
...
fitted.model <- readRDS('D:/Flo/JAGS_models/cbrachyurus_model.rds') # deleted after model output
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/cbrachyurus_model_diagnostics.pdf", param_page=3)
```
...
tmap::tmap_save(tm = time1MAP, filename = 'docs/figs/time1MAP_SR_cbrachyurus.svg', device = svglite::svglite)
tmap::tmap_save(tm = time2MAP, filename = 'docs/figs/time2MAP_SR_cbrachyurus.svg', device = svglite::svglite)
...
cbrachyurus.pal.pu_gn_bivd <- c4a("pu_gn_bivd", n=3, m=5)
cbrachyurus.pal <- c(t(apply(cbrachyurus.pal.pu_gn_bivd, 2, rev)))
...
if (length(absent_classes)==0){
  cbrachyurus.new.pal <- cbrachyurus.pal
} else cbrachyurus.new.pal <- cbrachyurus.pal[-c(absent_classes)]
...
diffMAP.SD
tmap::tmap_save(tm = diffMAP.SD, filename = 'docs/figs/diffMAP_SD_cbrachyurus.svg', device = svglite::svglite)
```
