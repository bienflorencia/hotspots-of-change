---
title: "Chrysocyon brachyurus Data Preparation"
format: 
  html:
    toc: true
    toc-location: right
...
Data preparation for modelling for *Chrysocyon brachyurus*, with the covariates `bio_10`, `bio_14`, `elev`, and `grass` (selected in the previous step).
...
PO_cbrachyurus_time1_raster <- terra::rast('data/species_POPA_data/PO_cbrachyurus_time1_raster.tif')
PO_cbrachyurus_time2_raster <- terra::rast('data/species_POPA_data/PO_cbrachyurus_time2_raster.tif')

# blobs
PA_cbrachyurus_time1_blobs <- readRDS('data/species_POPA_data/PA_cbrachyurus_time1_blobs.rds')
PA_cbrachyurus_time2_blobs <- readRDS('data/species_POPA_data/PA_cbrachyurus_time2_blobs.rds')
```
...
env <- c(bio[[c('bio_10','bio_14')]], grass, elev) %>% 
...
grass.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_unscaled[["grass"]]) +
...
PO_cbrachyurus.area <- terra::cellSize(PO_cbrachyurus_time1_raster[[1]], unit='m', transform=F) %>% 
  classify(cbind(NaN, NA)) %>% terra::values()

# calculate coordinates for each raster cell
PO_cbrachyurus.coords <- terra::xyFromCell(PO_cbrachyurus_time1_raster, cell=1:ncell(PO_cbrachyurus_time1_raster)) %>% as.data.frame()
names(PO_cbrachyurus.coords) <- c('X', 'Y')

# number of cells in the rasters (both the same)
PO_cbrachyurus.cell <- 1:ncell(PO_cbrachyurus_time1_raster)

# with the environmental variables values for each cell
PO_cbrachyurus.env <- env_100k_resampled[] # scaled values

# with the mean accessibility value for each cell
PO_cbrachyurus.acce <- acce_100k_resampled # scaled values

# calculate point count in each raster cell
PO_cbrachyurus_time1.count <- PO_cbrachyurus_time1_raster %>%
    classify(cbind(NaN, NA)) %>% terra::values() %>% as_tibble %>% 
  dplyr::select(count)

PO_cbrachyurus_time2.count <- PO_cbrachyurus_time2_raster %>%
    classify(cbind(NaN, NA)) %>% terra::values() %>% as_tibble %>% 
  dplyr::select(count)

PO_cbrachyurus.countries <- latam_countries %>% classify(cbind(NaN, NA))

# save time1 and time2 datasets 
PO_cbrachyurus_time1.model.data <- data.frame(PO_cbrachyurus.coords,
                                            pixel = PO_cbrachyurus.cell[],
                                            area = PO_cbrachyurus.area,
                                            pt.count = PO_cbrachyurus_time1.count,
                                            env = PO_cbrachyurus.env,
                                            acce = PO_cbrachyurus.acce[], 
                                            country = PO_cbrachyurus.countries[])

PO_cbrachyurus_time2.model.data <- data.frame(PO_cbrachyurus.coords,
                                            pixel = PO_cbrachyurus.cell[],
                                            area = PO_cbrachyurus.area,
                                            pt.count = PO_cbrachyurus_time2.count,
                                            env = PO_cbrachyurus.env,
                                            acce = PO_cbrachyurus.acce[], 
                                            country = PO_cbrachyurus.countries[])
...
head(PO_cbrachyurus_time1.model.data) %>% kable()
head(PO_cbrachyurus_time2.model.data) %>% kable()
```
...
PA_cbrachyurus.area.time1 <- as.numeric(PA_cbrachyurus_time1_blobs$blobArea) 
PA_cbrachyurus.area.time2 <- as.numeric(PA_cbrachyurus_time2_blobs$blobArea) 

PA_cbrachyurus.coords.time1 <- st_coordinates(st_centroid(PA_cbrachyurus_time1_blobs)) %>% as_tibble()
PA_cbrachyurus.coords.time2 <- st_coordinates(st_centroid(PA_cbrachyurus_time2_blobs)) %>% as_tibble()

PA_cbrachyurus.pixel.time1 <- terra::cells(PO_cbrachyurus_time1_raster, vect(st_centroid(PA_cbrachyurus_time1_blobs))) 
PA_cbrachyurus.pixel.time2 <- terra::cells(PO_cbrachyurus_time2_raster, vect(st_centroid(PA_cbrachyurus_time2_blobs))) 
...
PA_cbrachyurus.env.time1 <- terra::extract(x = env, y = vect(PA_cbrachyurus_time1_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()

PA_cbrachyurus.env.time2 <- terra::extract(x = env, y = vect(PA_cbrachyurus_time2_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()

PA_cbrachyurus_time1.model.data <- data.frame(pixel = PA_cbrachyurus.pixel.time1, 
                                            PA_cbrachyurus.coords.time1,
                                            area = PA_cbrachyurus.area.time1,
                                            presabs = PA_cbrachyurus_time1_blobs$presence,
                                            effort = PA_cbrachyurus_time1_blobs$effort,
                                            env = PA_cbrachyurus.env.time1[,2:5])

PA_cbrachyurus_time2.model.data <- data.frame(pixel = PA_cbrachyurus.pixel.time2, 
                                            PA_cbrachyurus.coords.time2,
                                            area = PA_cbrachyurus.area.time2,
                                            presabs = PA_cbrachyurus_time2_blobs$presence,
                                            effort = PA_cbrachyurus_time2_blobs$effort,
                                            env = PA_cbrachyurus.env.time2[,2:5])
...
head(PA_cbrachyurus_time1.model.data) %>% kable()
head(PA_cbrachyurus_time2.model.data) %>% kable()
```
...
saveRDS(PO_cbrachyurus_time1.model.data, 'data/species_POPA_data/data_cbrachyurus_PO_time1.rds')
saveRDS(PO_cbrachyurus_time2.model.data, 'data/species_POPA_data/data_cbrachyurus_PO_time2.rds')
saveRDS(PA_cbrachyurus_time1.model.data, 'data/species_POPA_data/data_cbrachyurus_PA_time1.rds')
saveRDS(PA_cbrachyurus_time2.model.data, 'data/species_POPA_data/data_cbrachyurus_PA_time2.rds')
```
...
env_PA <- terra::extract(x = env, 
                         y = rbind(vect(PA_cbrachyurus_time1_blobs), 
                                   vect(PA_cbrachyurus_time2_blobs)), 
                                     fun = mean, rm.na=T) %>% 
