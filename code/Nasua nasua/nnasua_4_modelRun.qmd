...
PA_time1 <- readRDS('data/data_speciesAbrev_PA_time1.rds') %>%
  filter(!is.na(cov.1) &!is.na(cov.2) & !is.na(cov.3) & !is.na(cov.4)) # remove NA's
PA_time2 <- readRDS('data/data_speciesAbrev_PA_time2.rds') %>%
  filter(!is.na(cov.1) &!is.na(cov.2) & !is.na(cov.3) & !is.na(cov.4)) # remove NA's

# Presence-only data
PO_time1 <- readRDS('data/data_speciesAbrev_PO_time1.rds') %>%
  filter(!is.na(cov.1) &!is.na(cov.2) & !is.na(cov.3) & !is.na(cov.4) & !is.na(acce) & !is.na(count)) # remove NA's
...
gam.formula <- formula(count ~ cov.1 + 
                               cov.2 + 
                               cov.3 + 
                               cov.4 +
                               offset(log(area)) +
...
PA.X <- cbind('(Intercept)'= 1, 
              'cov.1' = PA_time1_time2$cov.1,
              'cov.2' = PA_time1_time2$cov.2,
              'cov.3' = PA_time1_time2$cov.3,
              'cov.4' = PA_time1_time2$cov.4,
              jagam.PA.time1, 
              jagam.PA.time2)

PO.X <- cbind('(Intercept)'= 1, 
              'cov.1' = PO_time1_time2$cov.1,
              'cov.2' = PO_time1_time2$cov.2,
              'cov.3' = PO_time1_time2$cov.3,
              'cov.4' = PO_time1_time2$cov.4,
              jagam.PO.time1, 
              jagam.PO.time2)
