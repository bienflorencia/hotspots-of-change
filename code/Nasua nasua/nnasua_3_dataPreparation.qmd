---
title: "Nasua nasua Data Preparation"
format: 
  html:
    toc: true
    toc-location: right
...
PO_nnasua_time1_raster <- terra::rast('data/PO_nnasua_time1_raster.tif')
PO_nnasua_time2_raster <- terra::rast('data/PO_nnasua_time2_raster.tif')

# blobs
PA_nnasua_time1_blobs <- readRDS('data/PA_nnasua_time1_blobs.rds')
PA_nnasua_time2_blobs <- readRDS('data/PA_nnasua_time2_blobs.rds')
```
...
PO_nnasua.countries <- Latam.raster.countries %>% classify(cbind(NaN, NA))
...
PA_nnasua.env.time1 <- terra::extract(x = env, y = vect(PA_nnasua_time1_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()

PA_nnasua.env.time2 <- terra::extract(x = env, y = vect(PA_nnasua_time2_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()
...
saveRDS(PO_nnasua_time1.model.data, 'data/data_nnasua_PO_time1.rds')
saveRDS(PO_nnasua_time2.model.data, 'data/data_nnasua_PO_time2.rds')
saveRDS(PA_nnasua_time1.model.data, 'data/data_nnasua_PA_time1.rds')
saveRDS(PA_nnasua_time2.model.data, 'data/data_nnasua_PA_time2.rds')
```
