---
title: "Cerdocyon thous Model Outputs new"
format: 
  html:
    toc: true
    toc-location: right
...
cthous_expert_blob_time1 <- readRDS('data/cthous_expert_blob_time1.rds')
cthous_expert_blob_time2 <- readRDS('data/cthous_expert_blob_time2.rds')

PA_time1 <- readRDS('data/data_cthous_PA_time1.rds') %>%
  cbind(expert=cthous_expert_blob_time1) %>%
  filter(!is.na(env.bio_3) &!is.na(env.bio_4) & !is.na(env.tree) & !is.na(env.urban) & !is.na(expert)) # remove NA's
PA_time2 <- readRDS('data/data_cthous_PA_time2.rds') %>%
  cbind(expert=cthous_expert_blob_time2) %>% 
  filter(!is.na(env.bio_3) &!is.na(env.bio_4) & !is.na(env.tree) & !is.na(env.urban) & !is.na(expert)) # remove NA's

# Presence-only data
cthous_expert_gridcell <- readRDS('data/cthous_expert_gridcell.rds')

PO_time1 <- readRDS('data/data_cthous_PO_time1.rds') %>%
  cbind(expert=cthous_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.bio_3) &!is.na(env.bio_4) & !is.na(env.tree) & !is.na(env.urban) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
PO_time2  <- readRDS('data/data_cthous_PO_time2.rds')  %>%
  cbind(expert=cthous_expert_gridcell$dist_exprt) %>% 
  filter(!is.na(env.bio_3) &!is.na(env.bio_4) & !is.na(env.tree) & !is.na(env.urban) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
# diagnostics
ggmcmc::ggmcmc(fitted.model.ggs.b, file="docs/cthous_model_diagnostics.pdf", param_page=3)
```
...
### Standard deviation (SD) of the probability of occurrence of *Cerdocyon thous*
...
caterpiller.params <- fitted.model.ggs.b %>%
  ...
  mutate(Parameter=as.factor(ifelse(Parameter=='env.bio_3', 'isothermality',
                                    ifelse(Parameter=='env.urban', 'temperature seasonality',
                                           ifelse(Parameter=='env.tree', 'percentage of tree cover',
                                                  ifelse(Parameter=='env.bio_4', 'urban and built-up lands', Parameter)))))) %>% 
