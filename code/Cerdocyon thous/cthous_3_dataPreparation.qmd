...
Data preparation for modelling for *Cerdocyon thous*, with the covariates `bio3`, `bio4`, `tree`, and `urban` (selected in the previous step).
...
PO_cthous_time1_raster <- terra::rast('data/PO_cthous_time1_raster.tif')
PO_cthous_time2_raster <- terra::rast('data/PO_cthous_time2_raster.tif')

# blobs
PA_cthous_time1_blobs <- readRDS('data/PA_cthous_time1_blobs.rds')
PA_cthous_time2_blobs <- readRDS('data/PA_cthous_time2_blobs.rds')
```
...
The following covariates were chosen using the 'variableSelection' script:
 ...
 - `bio3`: Temperature Annual Range
 - `bio4`: Mean Temperature of Warmest Quarter
 - `tree`: Net Primary Production (tree)
 - `urban`: Percentage of No Tree Cover
...
urban <- rast(here::here('big_data/urban_high.tif'))
urban <- resample(urban, bio, method='bilinear') 
names(urban) <- 'urban'
...
env <- c(bio[[c('bio_3','bio_4')]], urban, tree) %>% 
...
urban.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["urban"]]) +
    tm_raster(palette = 'YlGnBu', midpoint = NA, style= "cont", n=20)  + 
...
bio_3.plot <- tm_graticules(alpha = 0.3) +  
    tm_shape(env_100k_resampled[["bio_3"]]) +
...
PO_cthous.countries <- Latam.raster.countries %>% classify(cbind(NaN, NA))
...
PA_cthous.env.time1 <- terra::extract(x = env, y = vect(PA_cthous_time1_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()

PA_cthous.env.time2 <- terra::extract(x = env, y = vect(PA_cthous_time2_blobs), fun = mean, rm.na=T) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.), NA, .))) %>% scale()
...
saveRDS(PO_cthous_time1.model.data, 'data/data_cthous_PO_time1.rds')
saveRDS(PO_cthous_time2.model.data, 'data/data_cthous_PO_time2.rds')
saveRDS(PA_cthous_time1.model.data, 'data/data_cthous_PA_time1.rds')
saveRDS(PA_cthous_time2.model.data, 'data/data_cthous_PA_time2.rds')
```
