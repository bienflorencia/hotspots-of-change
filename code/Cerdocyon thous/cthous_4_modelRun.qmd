...
PA_time1 <- readRDS('data/data_cthous_PA_time1.rds') %>%
  ...
  filter(!is.na(env.bio_10) &!is.na(env.bio_16) & !is.na(env.npp) & !is.na(env.tree) & !is.na(expert)) # remove NA's
...
PO_time1 <- readRDS('data/data_cthous_PO_time1.rds') %>%
  ...
  filter(!is.na(env.bio_10) &!is.na(env.bio_16) & !is.na(env.npp) & !is.na(env.tree) & !is.na(acce) & !is.na(count) & !is.na(expert)) # remove NA's
...
PA.X <- cbind('(Intercept)'= 1, 
              ...
              'env.bio_16' = PA_time1_time2$env.bio_16,
...
PO.X <- cbind('(Intercept)'= 1, 
              ...
              'env.bio_16' = PO_time1_time2$env.bio_16,
...
saveRDS(cthous_model, 'D:/Flo/JAGS_models/cthous_model.rds')
```
