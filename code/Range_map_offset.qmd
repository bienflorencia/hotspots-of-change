...
Values
  ...
  - buffer_PO = 100000 -> 100km
  - buffer_PA = 10000 -> 10km
  - buffer_expert = 150000 -> 150km
...
binary_expert_range <- function(data_PO_sf, data_PA_pre_sf, data_PA_pos_sf, 
                                expert_range_sf, Latam,
                                buffer_PO = 50000, buffer_PA = 50000, 
                                buffer_expert = 100000){
  # 1) Generate a buffer of 100km distance for presence-only data (PO)
  ...
  PA_blobs_buffer <- rbind(data_PA_pre_sf %>% filter(presence==1),
                           data_PA_pos_sf %>% filter(presence==1)) %>% 
...
hyagouaroundi_PA_pre_blobs <- readRDS('data/PA_hyagouaroundi_time1_blobs.rds')
hyagouaroundi_PA_pos_blobs <- readRDS('data/PA_hyagouaroundi_time2_blobs.rds')
...
hyagouaroundi_expert <- binary_expert_range(hyagouaroundi_PO_data, 
                                            hyagouaroundi_PA_pre_blobs,
                                            hyagouaroundi_PA_pos_blobs,
...
hyagouaroundi_expert_blob_pre <- st_join(hyagouaroundi_PA_pre_blobs, hyagouaroundi_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

hyagouaroundi_expert_blob_pos <- st_join(hyagouaroundi_PA_pos_blobs, hyagouaroundi_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(hyagouaroundi_expert_blob_pre, 'data/hyagouaroundi_expert_blob_pre.rds')
saveRDS(hyagouaroundi_expert_blob_pos, 'data/hyagouaroundi_expert_blob_pos.rds')
```
...
lpardalis_PA_pre_blobs <- readRDS('data/PA_lpardalis_time1_blobs.rds')
lpardalis_PA_pos_blobs <- readRDS('data/PA_lpardalis_time2_blobs.rds')
...
lpardalis_expert <- binary_expert_range(lpardalis_PO_data, 
                                            lpardalis_PA_pre_blobs,
                                            lpardalis_PA_pos_blobs,
...
### Presence-absence data
lpardalis_expert_blob_pre <- st_join(lpardalis_PA_pre_blobs, lpardalis_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

lpardalis_expert_blob_pos <- st_join(lpardalis_PA_pos_blobs, lpardalis_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(lpardalis_expert_blob_pre, 'data/lpardalis_expert_blob_pre.rds')
saveRDS(lpardalis_expert_blob_pos, 'data/lpardalis_expert_blob_pos.rds')
```
...
cthous_PA_pre_blobs <- readRDS('data/PA_cthous_time1_blobs.rds')
cthous_PA_pos_blobs <- readRDS('data/PA_cthous_time2_blobs.rds')
...
### Presence-absence data
cthous_expert_blob_pre <- st_join(cthous_PA_pre_blobs, cthous_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

cthous_expert_blob_pos <- st_join(cthous_PA_pos_blobs, cthous_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(cthous_expert_blob_pre, 'data/cthous_expert_blob_pre.rds')
saveRDS(cthous_expert_blob_pos, 'data/cthous_expert_blob_pos.rds')
```
...
cbrachyurus_PA_pre_blobs <- readRDS('data/PA_cbrachyurus_time1_blobs.rds')
cbrachyurus_PA_pos_blobs <- readRDS('data/PA_cbrachyurus_time2_blobs.rds')
...
cbrachyurus_expert <- binary_expert_range(cbrachyurus_PO_data, 
                                            cbrachyurus_PA_pre_blobs,
                                            cbrachyurus_PA_pos_blobs,
...
### Presence-absence data
cbrachyurus_expert_blob_pre <- st_join(cbrachyurus_PA_pre_blobs, cbrachyurus_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

cbrachyurus_expert_blob_pos <- st_join(cbrachyurus_PA_pos_blobs, cbrachyurus_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(cbrachyurus_expert_blob_pre, 'data/cbrachyurus_expert_blob_pre.rds')
saveRDS(cbrachyurus_expert_blob_pos, 'data/cbrachyurus_expert_blob_pos.rds')
```
...
ebarbara_PA_pre_blobs <- readRDS('data/PA_ebarbara_time1_blobs.rds')
ebarbara_PA_pos_blobs <- readRDS('data/PA_ebarbara_time2_blobs.rds')
...
ebarbara_expert <- binary_expert_range(ebarbara_PO_data, 
                                            ebarbara_PA_pre_blobs,
                                            ebarbara_PA_pos_blobs,
                                            ebarbara_IUCN, Latam)
...
### Presence-absence data
ebarbara_expert_blob_pre <- st_join(ebarbara_PA_pre_blobs, ebarbara_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

ebarbara_expert_blob_pos <- st_join(ebarbara_PA_pos_blobs, ebarbara_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(ebarbara_expert_blob_pre, 'data/ebarbara_expert_blob_pre.rds')
saveRDS(ebarbara_expert_blob_pos, 'data/ebarbara_expert_blob_pos.rds')
```
...
lwiedii_PA_pre_blobs <- readRDS('data/PA_lwiedii_time1_blobs.rds')
lwiedii_PA_pos_blobs <- readRDS('data/PA_lwiedii_time2_blobs.rds')
...
lwiedii_expert <- binary_expert_range(lwiedii_PO_data, 
                                            lwiedii_PA_pre_blobs,
                                            lwiedii_PA_pos_blobs,
                                            lwiedii_IUCN, Latam)
...
### Presence-absence data
lwiedii_expert_blob_pre <- st_join(lwiedii_PA_pre_blobs, lwiedii_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

lwiedii_expert_blob_pos <- st_join(lwiedii_PA_pos_blobs, lwiedii_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(lwiedii_expert_blob_pre, 'data/lwiedii_expert_blob_pre.rds')
saveRDS(lwiedii_expert_blob_pos, 'data/lwiedii_expert_blob_pos.rds')
```
...
nnasua_PA_pre_blobs <- readRDS('data/PA_nnasua_time1_blobs.rds')
nnasua_PA_pos_blobs <- readRDS('data/PA_nnasua_time2_blobs.rds')
...
### Presence-absence data
nnasua_expert_blob_pre <- st_join(nnasua_PA_pre_blobs, nnasua_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

nnasua_expert_blob_pos <- st_join(nnasua_PA_pos_blobs, nnasua_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(nnasua_expert_blob_pre, 'data/nnasua_expert_blob_pre.rds')
saveRDS(nnasua_expert_blob_pos, 'data/nnasua_expert_blob_pos.rds')
```
...
pbrasiliensis_PA_pre_blobs <- readRDS('data/PA_pbrasiliensis_time1_blobs.rds')
pbrasiliensis_PA_pos_blobs <- readRDS('data/PA_pbrasiliensis_time2_blobs.rds')
...
pbrasiliensis_expert <- binary_expert_range(pbrasiliensis_PO_data, 
                                            pbrasiliensis_PA_pre_blobs,
                                            pbrasiliensis_PA_pos_blobs,
...
### Presence-absence data
pbrasiliensis_expert_blob_pre <- st_join(pbrasiliensis_PA_pre_blobs, pbrasiliensis_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)

pbrasiliensis_expert_blob_pos <- st_join(pbrasiliensis_PA_pos_blobs, pbrasiliensis_expert[[2]] %>% mutate(presence=1)) %>% 
  mutate(presence=ifelse(is.na(presence.y), 0, presence.y)) %>% dplyr::select(presence)
...
saveRDS(pbrasiliensis_expert_blob_pre, 'data/pbrasiliensis_expert_blob_pre.rds')
saveRDS(pbrasiliensis_expert_blob_pos, 'data/pbrasiliensis_expert_blob_pos.rds')
```
